function rotateDim2d(vec: p5.Vector, dimX: number, dimY: number, theta: number) {
...
function bitRotateRight(x: number, shift: number, totalBits: number): number {
    shift %= totalBits;
...
class Hypercube {
    ...
    public rot: p5.Vector;
    public size: number;
    public moveSpeed: p5.Vector;
    public rotSpeed: p5.Vector;
    public color: number[];
    ...
    public draw() {
        ...
        for (let i = 0; i < 2 ** 4; i++) {
            let vec = createVector(
                ...
                (i & 0b0010) >> 1,
                (i & 0b0001) >> 0
            ).sub([0.5, 0.5, 0.5, 0.5]).mult(this.size);
...
let inkStrokeShader: p5.Shader;
let inkFramebuffer: p5.Framebuffer;
let inkyShader: p5.Shader;
