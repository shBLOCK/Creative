...
(function (Type) {
    ...
    Type[Type["Complex"] = 1] = "Complex";
})(Type || (Type = {}));
class Op {
    shaderTemplate;
    inputTypes;
    outputType;
    constructor(shaderTemplate, inputTypes, outputType) {
...
function genRandomFractalFunc(complexity_drop_off = 0.9) {
    function chooseOp(outputType) {
        while (true) {
            const op = random(Object.values(OPS));
            if (op.outputType == outputType)
                return op;
        }
    }
    function gen(outputType, complexity) {
...
let fractalFrameBuffer;
let baseFractalShader;
let fractalShader;
let timeAcc = 0.0;
...
function draw() {
    ...
    if (timeAcc > 1.1) {
        ...
        function isInfOrNanPixel(pixel) {
            return pixel[3] < 200;
        }
