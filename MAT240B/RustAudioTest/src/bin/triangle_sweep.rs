...
fn main() {
    let mut theta = 0f32;
    simple_audio::play_and_record_audio_function::<SR, 2, _>("output/triangle_sweep.wav", 12.8, move |t| {
        ...
        for n in (1..10000).step_by(2) {
            let mut amp = (1.0 / (n * n) as f32) * (1.0 - (n / 2 % 2 * 2) as f32);
            amp *= (bandlimit_n - n as f32).clamp(0.0, 2.0) / 2.0;
            if amp == 0.0 { break; }
            signal += f32::sin(n as f32 * theta) * amp;
        }
        signal * (8.0 / (PI * PI)) * FRAC_1_SQRT_2
    })
        .unwrap();
}
